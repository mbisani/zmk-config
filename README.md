...
Four layers, defined in [`config/sofle_choc_pro.keymap`](config/sofle_choc_pro.keymap). `LOWER` and `RAISE` are held via the left/right thumb keys; holding both together auto-activates `ADJUST`.

- **default (QWERTY)** — standard US-ANSI-ish letters/numbers, close to a stock keyboard so it stays easy to switch back and forth with the MacBook keyboard. Left thumb row: `GUI ALT CTRL LOWER ENTER`; right thumb row: `SPACE RAISE CTRL ALT GUI`. A `MUTE`/`PLAY` pair sits in the top-right corner of each half's inner column. Home-row mods on `A S D F` / `J K L ;` mirror the thumb-key order (`GUI ALT CTRL SHIFT`, pinky→index on the left, mirrored on the right) — hold for the modifier, tap for the letter. Tuned as balanced hold-taps restricted to cross-hand key presses (via `hold-trigger-key-positions`) so same-hand rolls like `as` or `fr` resolve as taps, not accidental modifiers.
