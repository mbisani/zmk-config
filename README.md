...
- **nav (NAV)** — arrow keys and `HOME`/`END`/`PGUP`/`PGDN`/`DEL`/`BSPC` under the right hand for navigation. Left half is all `&trans`, so the home-row mods fall through from `alpha` unchanged (e.g. hold `D` for `CTRL` + tap an arrow key) — no other bindings on the left half.
